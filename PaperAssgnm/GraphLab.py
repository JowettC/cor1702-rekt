    ...
    def run(self):
        ...
        def update():
            ...
            for i in range(1, len(self.updateArr) + 1):
                ...
                canvas.create_text(self.h[value][2][0] + 50, self.h[value][2][1] + 75, anchor='center', text=str(i),
                                   fill='red')
...
def greedy2(g, step=1000, visualize=False):
    ...
    while len(path) != len(g.vertices):
        ...
        # compare between A-D-B-C and A-B-D-C
            ...
            d1 = distance_dict[str(prev_in_path) + str(nearest_to_add)] + nearest_dist + distance_dict[
                str(nearest_in_path) + str(next_in_path)]

            # calculating distance of A-B-D-C
            d2 = distance_dict[str(prev_in_path) + str(nearest_in_path)] + nearest_dist + distance_dict[
                str(nearest_to_add) + str(next_in_path)]
...
def is_point_on_line(line, point):
    tempLine = Line(Point(0, 0),
                    Point(line.get_second().x - line.get_first().x, line.get_second().y - line.get_first().y))
